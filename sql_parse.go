...
func main() {
	...
	sql := `CREATE OR REPLACE FUNCTION func_name(res_table_name text, b IN OUT text) RETURNS SETOF RECORD AS $$ DECLARE tag text = '青少年';  BEGIN   EXECUTE 'CREATE TABLE ' || res_table_name || '(id serial, value text);'; EXECUTE 'INSERT INTO ' || res_table_name || '(value) VALUES('|| quote_literal(tag) ||');';  END  $$  LANGUAGE PLPGSQL   RETURNS NULL ON NULL INPUT;`
